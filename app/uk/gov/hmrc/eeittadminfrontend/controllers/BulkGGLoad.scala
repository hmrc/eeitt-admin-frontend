...
class BulkGGLoad(val authConnector: AuthConnector, eMACConnector: EMACConnector)(
  ...
  def getBulkloadPage = Authentication.async { implicit request =>
    Future.successful(
      Ok(uk.gov.hmrc.eeittadminfrontend.views.html.gg_bulkload(knownFactsForm, allocateEnrolment, listOfCredIds, "")))
  ...
  val listOfCredIds: Map[String, User] = if (switch) {
    Map(
      "USER1" -> pureconfig.loadConfigOrThrow[User]("feature.GGLoad.users.user1"),
      "USER2" -> pureconfig.loadConfigOrThrow[User]("feature.GGLoad.users.user2"),
      "USER3" -> pureconfig.loadConfigOrThrow[User]("feature.GGLoad.users.user3")
    )
  } else Map.empty[String, User]

  val knownFactsForm = Form(
    mapping(
      "enrollmentKey" -> mapping("service" -> nonEmptyText, "identifier" -> nonEmptyText, "value" -> nonEmptyText)(
        EnrollmentKey.apply)(EnrollmentKey.unapply),
      "verifiers" -> list(
        mapping("key" -> nonEmptyText, "value" -> nonEmptyText)(KeyValuePair.apply)(KeyValuePair.unapply))
    )(KnownFacts.apply)(KnownFacts.unapply))

  val allocateEnrolment = Form(
    mapping(
      "user" -> nonEmptyText,
      "enrolmentKey" -> mapping("service" -> nonEmptyText, "identifier" -> nonEmptyText, "value" -> nonEmptyText)(
        EnrollmentKey.apply)(EnrollmentKey.unapply),
      "verifiers" -> list(
        mapping("key" -> nonEmptyText, "value" -> nonEmptyText)(KeyValuePair.apply)(KeyValuePair.unapply)),
  ...
  def deleteEndPoint(): Action[AnyContent] = Authentication.async { implicit request =>
    deleteForm.bindFromRequest.fold(
      errors => Future.successful(BadRequest("Ok")),
      success => {
        val user = listOfCredIds(success.user)
        for {
