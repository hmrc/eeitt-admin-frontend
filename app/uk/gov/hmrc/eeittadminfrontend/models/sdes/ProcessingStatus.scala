...
object ProcessingStatus {
  ...
  case object Succeeded extends ResultStatus {
    override val name = "succeeded"
  }

  case object Deferred extends ResultStatus {
    override val name = "deferred"
  }
  ...
  val reProcessable: Set[ProcessingStatus] =
    Set(
      PermanentlyFailed,
      Ignored,
      Deferred
  ...
  def fromName(name: String): Option[ProcessingStatus] =
    nameToStatus.find(m => m._1 === name).map(_._2)
}
