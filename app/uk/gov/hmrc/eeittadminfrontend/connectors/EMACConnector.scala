...
trait EMACConnectorHelper {
  ...
  def assignEnrollment(enrollment: Enrollment, user: User)(implicit hc: HeaderCarrier, ec: ExecutionContext): Future[Option[JsValue]] = {
    allocateAnEnrollment(enrollment, user).map(result).flatMap {
      case None => {
        POST.POSTEmpty[HttpResponse](s"$ES11url${user.credId}/enrolments/${enrollment.enrollmentKey.service}~${enrollment.enrollmentKey.identifier}~${enrollment.enrollmentKey.value}?legacy-credentialId=${user.credId}").map(result).map{
          case None => None
          case x =>
