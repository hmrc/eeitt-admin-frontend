...
class GformsController(val authConnector: AuthConnector)(implicit appConfig: AppConfig, val messagesApi: MessagesApi) extends FrontendController with Actions with I18nSupport {
  ...
  def uploadGformSchema = Authentication.async(parse.multipartFormData) { implicit request =>
    request.body.file("template").map { x =>
      val template = Json.parse(Source.fromFile(x.ref.file).getLines().mkString)
    ...
    }.getOrElse(Future.successful(Ok("Failed")))
  }
